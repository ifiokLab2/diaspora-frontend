  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
     ...
     await login({ email, password }, 'seller');
      ...
      setError(error.error || 'Invalid credentials for seller account.');
      ...
      <div className="w-full max-w-sm rounded-xl bg-background px-8 py-6 shadow-lg">
        ...
        <form className="mt-8 space-y-5" onSubmit={ handleSubmit}>
          ...
          <p className="text-center text-sm text-muted-foreground">
            ...
            <Link href="/signup/seller/" className="font-medium text-brand hover:underline">
              Signup
            </Link>
          </p>
          <p className="text-gray-600">Standard user? <Link href="/login/customer" className="text-emerald-600 font-semibold hover:underline">Customer Login</Link></p>
        </form>
      </div>
    </div>
  );
